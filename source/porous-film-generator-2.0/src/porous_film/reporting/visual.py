...
def _input_check_payload(
    ...
    formal_complete = bool(
        config.formal_targets.position_quantity.center_distance_xy is not None
        and all(value is not None for value in required_shape_targets)
    )
    formal_status = (
        "PASS"
        if config.source_schema_version == 3 and formal_complete
        else "FAIL"
        if config.source_schema_version == 3
        else "SKIP"
    )
    ...
    return [
        ...
        _check_row(
            "Schema v3 必填正式目标",
            formal_status,
            "位置、孔隙率、尺寸、成对方向、通道 eta/tau、曲率波动",
            "schema-v3 输入的所有正式目标必须存在",
            "全部存在。"
            if formal_status == "PASS"
            else "至少一个正式目标缺失。"
            if formal_status == "FAIL"
            else "当前是兼容输入，不适用 schema-v3 必填项检查。",
        ...
        _check_row(
            "可视化输出",
            "PASS" if config.output.write_plots else "SKIP",
...
def _observation_check_payload(
    ...
    rows = [
        ...
        _check_row(
            "z 贯通中心线",
            "PASS" if through_count > 0 else "FAIL",
            str(through_count),
            "> 0（诊断门；正式形状样本只取贯通轨迹）",
            "存在贯通轨迹。"
            if through_count > 0
            else "没有 z 贯通轨迹，后续形状、方向和通道指标没有有效正式样本。",
            category="贯通性",
            gate=False,
        ),
    ]
    ...
    for distribution in distributions:
        ...
        rows.append(
            _check_row(
                ...
                "SKIP" if not required else "PASS" if comparison_passed else "FAIL",
                (
    ...
    rows.extend(
        [
            ...
            _check_row(
                ...
                (
                    "满足配置值；当前 schema-v3 实现将其作为诊断，不参与总体正式审核。"
                    if audit.overlap_fraction
                    <= config.matrix_constraints.maximum_overlap_fraction
                    else "超过配置值；当前 schema-v3 实现尚未把该项纳入总体正式审核。"
                ),
                category="几何诊断",
                gate=False,
            ),
...
<main>
  ...
  <section id="validation" hidden>
    ...
    <div class="grid" id="validationCharts" style="margin-top:16px"></div>
...
function auditStatus(s){{const key=String(s||'SKIP').toLowerCase();return `<span class="audit-status ${{key}}">${{escapeHtml(String(s||'SKIP'))}}</span>`}}
function auditRows(rows){{const rank={{FAIL:0,WARN:1,PASS:2,SKIP:3}},ordered=[...rows].sort((a,b)=>(rank[a.status]??9)-(rank[b.status]??9));return ordered.map(q=>`<tr><td>${{auditStatus(q.status)}}</td><td>${{escapeHtml(q.category)}}</td><td>${{escapeHtml(q.name)}}</td><td>${{escapeHtml(q.observed)}}</td><td>${{escapeHtml(q.requirement)}}</td><td>${{q.gate?'正式门槛':'诊断'}}</td><td>${{escapeHtml(q.reason)}}</td></tr>`).join('')}}
